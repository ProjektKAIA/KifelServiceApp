// src/screens/employee/ScheduleScreen.tsx
...
const mockShifts = [
  ...
  { date: '2024-12-06', startTime: '08:00', endTime: '16:00', location: 'Standort Forst' },
  { date: '2024-12-08', startTime: '08:00', endTime: '16:00', location: 'Standort Forst' },
  { date: '2024-12-11', startTime: '08:00', endTime: '16:00', location: 'Standort Forst' },
...
export default function ScheduleScreen() {
  ...
  const calendarDays = useMemo(() => {
    ...
    // Füge leere Tage am Anfang hinzu (Woche beginnt mit Montag)
    const startDay = start.getDay();
    const emptyDays = startDay === 0 ? 6 : startDay - 1;
    const paddedDays = Array(emptyDays).fill(null).concat(days);
              ...
              return (
                <View key={index} style={styles.dayCell}>
                  {day && (
                    <View style={[
                      styles.dayContent,
                      shift && styles.dayWithShift,
                      shift && { backgroundColor: 'rgba(59, 130, 246, 0.15)' },
                      isCurrentDay && styles.dayToday,
                    ]}>
                      <Text style={[
                        styles.dayNumber,
                        { color: theme.textMuted },
                        shift && { color: theme.primary, fontWeight: '600' },
                        isCurrentDay && { color: '#fff', fontWeight: '600' },
                      ]}>
                        {format(day, 'd')}
                      </Text>
                    </View>
            ...
            >
              <View style={styles.shiftInfo}>
                <Text style={[styles.shiftDate, { color: theme.text }]}>
                  {isToday(shiftDate) 
                    ? `Heute, ${format(shiftDate, 'd. MMM', { locale: de })}`
                    : format(shiftDate, 'EEE, d. MMM', { locale: de })
                  }
                </Text>
                ...
                <View style={styles.shiftLocation}>
                  ...
                  <Text style={[styles.shiftLocationText, { color: theme.primary }]}>
                    {shift.location}
                  </Text>
                </View>
              </View>
              <View style={[styles.statusPill, { 
                backgroundColor: isActive ? theme.pillSuccess : theme.pillInfo 
              }]}>
                <View style={[styles.statusDot, { 
                  backgroundColor: isActive ? theme.pillSuccessText : theme.pillInfoText 
                }]} />
                <Text style={[styles.statusText, { 
                  color: isActive ? theme.pillSuccessText : theme.pillInfoText 
                }]}>
